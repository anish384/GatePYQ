...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const closeCalcBtn = document.getElementById('closeCalculatorBtn');

    document.getElementById('calculatorBtn').addEventListener('click', function () {
        if (calcOverlay) {
            calcOverlay.style.display = calcOverlay.style.display === 'none' ? 'flex' : 'none';
        }
    });
...
function loadSectionTabs() {
    ...
    examData.sections.forEach((section, index) => {
        ...
        tab.innerHTML = `${section.name} <span class="tab-count">${section.questions.length}</span>`;
...
function loadQuestion() {
    ...
    document.getElementById('questionMarks').textContent =
        `Marks for correct answer: ${question.marks} | Negative Marks: ${question.negative_marks || 0}`;

    // Update section pill
    const sectionPill = document.getElementById('currentSectionPill');
    if (sectionPill) sectionPill.textContent = section.name;
...
function updateQuestionPalette() {
    ...
    buttons.forEach((btn) => {
        ...
        if (state.marked && state.answered) {
            ...
            btn.innerHTML = `<span>${displayQId}</span><span style="position:absolute; bottom:-4px; right:-4px; background:green; color:white; font-size:8px; padding:1px; border-radius:50%;">✔</span>`;
        ...
        section.questions.forEach(question => {
            ...
            if (state.answered) {
                if (question.type === 'multiple') {
                    ...
                    if (correctValRaw.includes('-') && !correctValRaw.startsWith('-')) {
